...
import EditModifier from './editModifier/editModifier';
import { useModifiersStore } from '@/zstore/modifiers.store';
...
export default function Modificaciones() {
  ...
  const [showModal, setShowModal] = useState<ShowModalOptions>(ShowModalOptions.INITIAL_STATE);
  ...
  const confirmUpdateModifier = useModal('confirmUpdateModifier');
  // Events
  ...
  const updateModifierAction = useModifiersStore((state)=> state.updateModifier);
  const updateModifierLoading = useModifiersStore((state)=> state.isLoading);
  const updateModifierError = useModifiersStore((state)=> state.error);
    ...
    <div className={styles.container}>
      <section className={styles.head}>
        ...
        <div>
          ...
          {
            confirmUpdateModifier.isOpen && confirmUpdateModifier.modalName === 'confirmUpdateModifier' && <ConfirmChangesModal
              loading={updateModifierLoading}
              errors={updateModifierError}
              isOpen={confirmUpdateModifier.isOpen}
              onClose={confirmUpdateModifier.closeModal}
              closeModal={()=> {
                setShowModal(ShowModalOptions.INITIAL_STATE);
              }}
            >
              Cambios guardados
            </ConfirmChangesModal>
      ...
      <section className={styles.mainSection}>
                  ...
                  {element.status === 'enabled' ? (
                    <>
                      <button className={styles.actionButtonsFirst} onClick={() => {
                        ...
                        setShowModal(ShowModalOptions.EDIT_MODIFIER)
                      }}>
        ...
        {
            showModal === ShowModalOptions.EDIT_MODIFIER && <EditModifier modifier={selectedModifier} onClose={() => setShowModal(ShowModalOptions.INITIAL_STATE)} action={updateModifierAction} openModal={confirmUpdateModifier.openModal} />
        }
